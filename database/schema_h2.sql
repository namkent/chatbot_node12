...
-- Mô tả: DDL khởi tạo toàn bộ các bảng lưu trữ Chat Sessions, Messages, 
--        Dynamic Tools và Kho Tri Thức RAG (Knowledge Bases & Chunks)
-- ==============================================================================

-- 1. BẢNG PHIÊN TRÒ CHUYỆN (CHAT SESSIONS)
-- Lưu trữ thông tin từng phiên hội thoại của người dùng
...
-- Lưu trữ chi tiết từng tin nhắn (user, bot/assistant) trong từng phiên chat
...
-- Dành cho Admin cấu hình các Tools / Function Calling gọi mở rộng
...
-- Quản lý các nhóm tri thức theo chuyên ngành / tài liệu
...
-- Chứa nội dung toàn văn của các tài liệu tải lên (Markdown, Text, File)
...
-- Phục vụ tìm kiếm ngữ nghĩa Vector Search / TF-IDF Keyword Matching
...
-- ==============================================================================
-- DỮ LIỆU MẪU KHỞI TẠO (SAMPLE DATA)
-- ==============================================================================

-- Thêm phiên chat mẫu cho người dùng giả lập 'nam.dovan'
INSERT INTO chat_sessions (id, user_id, user_name, title, emoji, is_custom_renamed, model, created_at, updated_at)
VALUES 
('sample001a1b', 'nam.dovan', 'Do Van Nam', 'Giới thiệu Trợ lý AI', '🚀', FALSE, 'openai/gpt-oss-120b', 1789310000000, 1789310000000);

INSERT INTO chat_messages (session_id, user_id, role, content, order_idx, created_at)
VALUES 
('sample001a1b', 'nam.dovan', 'user', 'Xin chào, bạn có thể làm được những gì?', 0, 1789310001000),
('sample001a1b', 'nam.dovan', 'assistant', 'Chào bạn **Do Van Nam**! Tôi là **Astro Assistant** – trợ lý thông minh hỗ trợ giải đáp kỹ thuật, viết code, vẽ sơ đồ Mermaid, quản lý tài liệu tri thức RAG và tự động lưu phiên làm việc an toàn trên H2 Database.', 1, 1789310002000);
